...
from app.models.purchase import PurchaseModel, StatusTypeEnum
import logging
...
@router.get("/get-purchases", response_model=List[PurchaseModel])
async def get(
...
async def get_by_id(purchase_id: int, db: AsyncSession = Depends(get_db)) -> PurchaseModel:
    """
    Args:
        purchade_id: Id from the purchase the user wants to retrieve
...
@router.get("/get-purchase-by-product/{product_id}")
async def get_by_product_id(product_id: int, db: AsyncSession = Depends(get_db)) -> PurchaseModel:
    """
    Args:
        purchade_id: Id from the purchase the user wants to retrieve
    ...
    Returns:
        The purchase with the id the user entered
    """
    logger.info(f"Trying to fetch a purchase by the id: {product_id}")
